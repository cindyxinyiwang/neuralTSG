...
class GRU_TSG_layer(Layer):
	...
	def decode_next(self, c, state, par_state, emb):
		'''
			Get the next hidden state. Used in beam search and sampling. 
		...
		gate_c = tensor.dot(c, self.gate_context)
		gate_before	= tensor.dot(state, self.gate_hidden)
		gate_par_in = tensor.dot(par_state, self.gate_par_in)
		gate = tensor.nnet.sigmoid(gate_in + gate_before + gate_c + gate_par_in)

		reset_c = tensor.dot(c, self.reset_context)
		reset_before = tensor.dot(state, self.reset_hidden)
		reset_par_in = tensor.dot(par_state, self.reset_par_in)
		reset = tensor.nnet.sigmoid(reset_in + reset_before + reset_c + reset_par_in)

		newstate = state_in + tensor.dot(c, self.input_context) + \
				tensor.dot(reset * state, self.input_hidden) + tensor.dot(par_state, self.input_par_in)

		newstate = self.active(newstate)
		newstate = (1 - gate) * state + gate * newstate
		return newstate

	def forward_step(self, t, state_before, state_hist, state_in, gate_in, reset_in, parent_t,\
					context, att_c, mask = None, cmask = None):
		'''
			Build the one-step computational graph which calculates the next hidden state.
		...
		# parent state, state_hist (timestep, batch_size, dim)
		timestep, batch_size, dim = state_hist.shape
		#par_state_idx = parent_t * batch_size + tensor.arange(batch_size, dtype='int64') # size: (batch_size,)
		par_state_idx = parent_t 
		par_state = state_hist.reshape((timestep * batch_size, dim))[par_state_idx, :]
		par_state = par_state.reshape(state_before.shape) # size (batch_size, dim)
		...
		reset_par_in = tensor.dot(par_state, self.reset_par_in)
		reset_in += reset_cin
		reset_in += reset_par_in
		reset_preactive = tensor.dot(state_before, self.reset_hidden) + reset_in
		reset = tensor.nnet.sigmoid(reset_preactive)
		
		state_cin = tensor.dot(c, self.input_context)
		state_par_in = tensor.dot(par_state, self.input_par_in)
		state_in += state_cin
		state_in += state_par_in
		...
		new_state_hist = tensor.set_subtensor(state_hist[t, :, :], h)
		...
		if self.verbose:
			return newstate, new_state_hist, c, attention, gate_cin, gate_preactive, gate, reset_cin, \
					reset_preactive, reset, state_cin, reseted, preactive, state
		else:
			return newstate, new_state_hist, c, attention
	...
	def forward(self, emb_in, length, context, state_init, parent_t_seq, batch_size = 1, mask = None, cmask = None):
		...
		#theano.printing.debugprint(gate_in.shape)
		#theano.printing.debugprint(length)
		#theano.printing.debugprint(batch_size)
		#theano.printing.debugprint(dim)
...
class LayerFactory(object):
	...
	def createGRU_tsg(self, name, dim_in, dim_c, dim, dim_class, **args):
		newLayer = GRU_TSG_layer(name, dim_in, dim_c, dim, dim_class, **args)
		self.layers += [newLayer]
		self.params += newLayer.params 
		return newLayer

...
def getbatch(lx, ly, config):
	...
	# check if ly is of type [ [[], []], [[]] ]; that is each item in ly is a list of lists
	y_is_list = isinstance(ly[0][0], list)
	y_dim = len(ly[0][0])

	# get the length of longest sentence in one batch
	if y_is_list:
		# Do not pad eos for y if y is TSG
		xlen = min(max([len(i) for i in lx]), config['maxlength'])
		ylen = min(max([len(i) for i in ly]), config['maxlength']) 
	else:
	...
	for i in range(len(lx)):
		if len(lx[i]) <= config['maxlength'] and len(ly[i]) <= config['maxlength']:
			...
			if y_is_list:
				#ty.append(numpy.concatenate( (ly[i], [ [config['index_eos_trg']] * y_dim ])))
				tx.append(lx[i])
				ty.append(ly[i])
			else:
	...
	if y_is_list:
		y = config['index_eos_trg'] * numpy.ones((ylen, len(ty), y_dim), dtype = 'int64')
	else:
...
class DataCollection(object):
	...
	def __init__(self, config, train = True):
		...
		self.updatetime = 0.
		self.train_cost = []
		self.status_variable = ['next_offset', 'num_iter', 'time', 'updatetime', 'valid_result', 'train_cost']
		self.load_data()
	...
	def load_data(self):
		...
		logging.info('discard long sentences, remaining %d sentences' % len(self.source))
	...
	def load_status(self, path):
		...
		try:
			...
			logging.info('no status file, start from scratch')

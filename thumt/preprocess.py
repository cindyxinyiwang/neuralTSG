...
def preprocess(num, vocab_f, ivocab_f, input_f, output_f, data_vocab = 'cPickle', data_corpus = 'json', withdict = False, fromgr = False):
	'''
		Count the most frequent words in corpus.
		Then, generate the vocabulary file and transform corpus into index sequences. 
	...
	#indexize corpus
	logging.info('indexizing corpus...')
	input = open(input_f, 'r')
	line = input.readline()
	corpora = []
	num_words = 0
	...
	if not withdict:
		logging.info('total %d unique words out of %d sentences' % (len(count_sort), num_sentences))
	logging.info('perserving %d words (including UNK and empty symbol), covering %2f of the words' % (len(vocab), 100. * (num_words - UNKs) / num_words))
...
def shuffle(src, trg, src_shuf, trg_shuf, data_corpus = 'json'):
	'''
		Random shuffle the parallel corpus
...
def shuffle_mono(src, src_shuf, data_corpus = 'json'):
	'''
		Random shuffle the monolingual corpus
...
if __name__ == "__main__":
	...
	logging.info('STEP 1: Process\n')
	if os.path.exists(cfg['src_shuf']) and os.path.exists(cfg['trg_shuf']):
		logging.info('File exist, skipped\n')
	else:
		if not args.nopreprocess:
			logging.info('STEP 1.1: processing source corpora...')
			preprocess(cfg['index_eos_src'], 
					...
					args.fromgr)
			logging.info('Done\n')
			logging.info('STEP 1.2: processing target corpora...')
	...
	if cfg['src_mono_shuf'] != '':
		if os.path.exists(cfg['src_mono_shuf']):
			logging.info('Monolingual source file exist, skipped\n')
		else:
			logging.info('STEP 1.4: processing monolingual source corpora...')
			preprocess(cfg['index_eos_src'], 
					cfg['vocab_src'], 
					cfg['ivocab_src'], 
					cfg['src_mono_text'], 
	...
	if cfg['trg_mono_shuf'] != '':
		if os.path.exists(cfg['trg_mono_shuf']):
			logging.info('Monolingual target file exist, skipped\n')
		else:
			logging.info('STEP 1.5: processing monolingual target corpora...')
			preprocess(cfg['index_eos_trg'], 
					cfg['vocab_trg'], 
					cfg['ivocab_trg'], 
					cfg['trg_mono_text'], 

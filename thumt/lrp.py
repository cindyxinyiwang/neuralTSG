...
parser.add_argument('-s', '--sentence-file', required = True, help = 'path to sentence')
parser.add_argument('-i', '--input-valfile', required = True, help = 'path to input value file')
...
def init_idx(size, name, shared = True):
    '''
        Initialize words indice in sentence.
    '''
...
class DecoderVal(object):
    '''
        The class which stores the intermediate variables produced by decoder in NMT model.
...
class Model(object):
    ...
    def cal_decoder(self, R_x):
        ...
        for i in range(self.trg_word_num):
            ...
            R_c_x.append(c)
            out.append(f(i, c))
        print 'calculate relevance bewteen context and x'
        R_c_x = numpy.array(R_c_x, dtype = 'float32')
        
        print 'calculate relevance bewteen decoder hidden states and x'
        ...
        print 'calculate relevance bewteen readout and x'
        ...
        print 'calculate relevance bewteen decoder hidden states and y'        
        ...
        print 'calculate relevance bewteen readout and y'
...
if __name__ == "__main__":
    ...
    f1 = open(args.sentence_file)
    f = open(args.output_file, 'w')
    src = f1.readline().strip() 
    trg = f1.readline().strip()
    f.write(src + '\n')
    f.write(trg + '\n')
    print 'init model'
    model = Model(len(src.split()), len(trg.split()), config)
    print 'read Data'
    model.readData(args.model, args.input_valfile)

    print 'calculate encoder relevance'
    R_enc_forward = model.cal_encoder()
    R_enc_back = model.cal_back_encoder()
    R_enc = numpy.concatenate((R_enc_forward, R_enc_back), axis = 2)
    
    print 'calculate decoder relevance'
    R_c_x, R_h_x, R_o_x, R_h_y, R_o_y = model.cal_decoder(R_enc)
    
    print 'normalize and store'
    normalize(R_enc_forward, f)
    normalize(R_enc_back, f)
    normalize(R_enc, f)
    normalize(R_c_x, f)
    normalize(R_h_x, f)
    normalize(R_h_y, f)
    save(model.decoder_val.dec_att.get_value(), f)
    normalize(R_o_x, f)
    normalize(R_o_y, f)

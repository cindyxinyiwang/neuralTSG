...
class BiRNNsearch(model):
	...
	def build(self):
		...
		# merge parameters and objectives
	...
	def get_addition_grads(self, cost0, cost1, now_s):
		'''
			Update the total cost of bidirectional NMT
		'''
		bs = self.config['batchsize']
		sample_n = self.config['sample_num']
		sum_cost_0 = cost0[bs : bs + bs * sample_n] * self.config['auto_lambda_1'] + cost1[bs : bs + bs * sample_n] * self.config['auto_lambda_2']
		sum_cost_1 = cost0[bs + bs * sample_n :] * self.config['auto_lambda_2'] + cost1[bs + bs * sample_n :] * self.config['auto_lambda_1']
	...
	def get_inputs_batch(self, xp, yp, xm, ym):
		'''
			Get a batch for semi-supervised training.
	...
	def sample(self, x, length, n_samples = 1):
		'''
			Sample with source-to-target network.
	...
	def sample_inv(self, x, length, n_samples = 1):
		'''
			Sample with target-to-source network.
